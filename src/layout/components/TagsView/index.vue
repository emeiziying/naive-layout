    ...
    <div class="tabs-view-main">
      <div ref="navWrap" class="tabs-card" :class="{ 'tabs-card-scrollable': scrollable }">
        ...
        <div ref="navScroll" class="tabs-card-scroll">
          <Draggable animation="300" class="flex" item-key="fullPath" :list="tabsList">
...
export default defineComponent({
  ...
  setup(props) {
    ...
    // 移除缓存组件名称
    const delKeepAliveCompName = (route: RouteLocationNormalizedLoadedGeneric) => {
      if (route.meta.keepAlive) {
        const matchedRoute = router.currentRoute.value.matched.find(
          (item) => item.name == route.name,
        )
        const name = matchedRoute?.components?.default.name
        if (name) {
          const matchedTabs = tabsList.value.filter((item) => item.name == matchedRoute.name)
    ...
    watch(
      ...
      (to) => {
        ...
        const { meta, ...rest } = getSimpleRoute(route)

        tabsViewStore.addTab({
          ...rest,
          meta: { ...meta, title: title ? `${meta.title}-${title}` : meta.title },
        })
    ...
    .tabs-card {
      ...
      &-scroll {
        ...
        .active-item {
          ...
          background: linear-gradient(180deg, #007aff4d 0%, #ffffff00 100%);
        }
      }
    }
