...
export const createApis = (alovaInstance: Alova<AlovaGenerics>, configMap: any) => {
  const Apis = new Proxy({} as Auth, {
    get(_, property) {
      return createFunctionalProxy([property], alovaInstance, configMap);
    }
  });
  // define global variable `Apis`
  (globalThis as any).Auth = Apis;
  return Apis;
};
type MethodConfig<T> = AlovaMethodCreateConfig<
  ...
  T
>;
type APISofParameters<Tag extends string, Url extends string> = Tag extends keyof Auth
  ? Url extends keyof Auth[Tag]
    ? Auth[Tag][Url] extends (...args: any) => any
      ? Parameters<Auth[Tag][Url]>
      : any
    : any
  : any;
