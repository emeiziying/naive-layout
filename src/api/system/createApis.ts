...
export const createApis = (alovaInstance: Alova<AlovaGenerics>, configMap: any) => {
  const Apis = new Proxy({} as System, {
    get(_, property) {
      return createFunctionalProxy([property], alovaInstance, configMap);
    }
  });
  // define global variable `Apis`
  (globalThis as any).System = Apis;
  return Apis;
};
type MethodConfig<T> = AlovaMethodCreateConfig<
  ...
  T
>;
type APISofParameters<Tag extends string, Url extends string> = Tag extends keyof System
  ? Url extends keyof System[Tag]
    ? System[Tag][Url] extends (...args: any) => any
      ? Parameters<System[Tag][Url]>
      : any
    : any
  : any;
